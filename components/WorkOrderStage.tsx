...
export const WorkOrderStage: React.FC<WorkOrderStageProps> = ({ 
  ...
  useEffect(() => {
      ...
      if (results.openCellSets > 0) {
          defaultLines.push({ id: 'oc', item: 'Open Cell Foam', description: 'Load Sets', qty: `${results.openCellSets.toFixed(2)} Sets`, amount: 0 });
      }
      if (results.closedCellSets > 0) {
          defaultLines.push({ id: 'cc', item: 'Closed Cell Foam', description: 'Load Sets', qty: `${results.closedCellSets.toFixed(2)} Sets`, amount: 0 });
      }
